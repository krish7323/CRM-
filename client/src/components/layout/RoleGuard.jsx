...
export const RoleGuard = ({ allowedRoles, children }) => {
  ...
  // Owner, Admin, and Owner/Admin have top-level executive access to all institute modules
  const isAdminOrOwner = currentUser.role === 'Owner' || currentUser.role === 'Admin' || currentUser.role === 'Owner/Admin';
  const isAllowed = allowedRoles.includes(currentUser.role) || isAdminOrOwner;
        ...
        <div className="w-full max-w-md glass-panel border border-rose-800/60 rounded-3xl p-8 text-center space-y-4 shadow-2xl">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Your account <strong>{currentUser.name}</strong> is assigned the role <strong>{currentUser.role}</strong>.
            </p>
          </div>
          ...
          <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 text-[11px] text-slate-400">
            ...
            <div className="flex flex-wrap justify-center gap-1 mt-1.5 font-semibold text-cyan-400">
              {allowedRoles.map((r) => (
                <span key={r} className="bg-slate-900 px-2 py-0.5 rounded border border-slate-800">
                  {r}
                </span>
              ))}
            </div>
