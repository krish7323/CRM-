...
import { Lock, Mail, Shield, ArrowRight, Eye, EyeOff, AlertTriangle } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const fillQuickCredentials = (emailVal: string) => {
  ...
  return (
    <div className="min-h-screen w-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      {/* Main Container */}
      <div className="w-full max-w-4xl glass-panel border border-slate-800/80 rounded-3xl overflow-hidden shadow-2xl grid grid-cols-1 md:grid-cols-2 relative z-10">
        ...
        <div className="bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 p-8 flex flex-col justify-between border-b md:border-b-0 md:border-r border-slate-800">
          <div>
            <div className="flex items-center space-x-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-amber-400 font-semibold">School Management System (4-Role Scope)</p>
              </div>
            </div>

            <h2 className="text-2xl font-black text-slate-100 leading-tight">
              Sign In to Your IIA Account
            </h2>
            <p className="text-xs text-slate-400 mt-3 leading-relaxed">
              Role-based access enforced across 4 roles: <strong>Admin</strong>, <strong>Counsellor</strong>, <strong>Teacher</strong>, and <strong>Student</strong>.
            </p>
          </div>
          ...
          <div className="mt-8 pt-6 border-t border-slate-800/80">
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-1.5">
              <Shield className="w-3.5 h-3.5 text-amber-400" /> IIA 4-Role Quick Test Accounts
            </p>
            ...
            <div className="grid grid-cols-2 gap-2">
              {users.map((u) => (
                <button
                  ...
                  className="p-2 rounded-xl bg-slate-900/90 hover:bg-slate-800 border border-slate-800 text-left transition flex items-center justify-between group"
                >
                  <div>
                    <p className="text-[11px] font-bold text-slate-200 group-hover:text-amber-300 truncate">{u.name}</p>
                    <p className="text-[9px] text-amber-400 font-medium">{u.role}</p>
                  </div>
                  <ArrowRight className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition text-amber-400 shrink-0" />
                </button>
              ))}
            </div>
          </div>
