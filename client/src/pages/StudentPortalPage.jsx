...
import { Calendar, CheckCircle2, Download, FileText, Upload, X, ShieldCheck, } from 'lucide-react';
export const StudentPortalPage = () => {
    ...
    const student = students.find((s) => s.email.toLowerCase() === currentUser.email.toLowerCase() || s.name === currentUser.name) ||
        students[0] || {
        studentId: 'IIA-1001',
    ...
    return (<div className="space-y-6 font-sans">
      ...
      <div className="glass-panel p-6 rounded-3xl border border-slate-800 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center space-x-4">
          <img src={currentUser.avatarUrl || 'https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?w=150'} alt={student.name} className="w-16 h-16 rounded-full border-2 border-amber-500/50 object-cover shadow-xl"/>
          <div>
        ...
        {(activeRole === 'Teacher' || activeRole === 'Admin') && (<button onClick={() => setIsNoteModalOpen(true)} className="flex items-center space-x-2 bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 font-bold text-xs px-4 py-2.5 rounded-xl shadow-lg shadow-amber-500/20 hover:scale-105 transition">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {studyNotes.map((note) => (<div key={note._id} className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2 flex flex-col justify-between">
              <div>
              ...
              <div className="pt-3 border-t border-slate-900 flex items-center justify-between text-[11px]">
                ...
                <a href={note.pdfUrl || '#'} download className="flex items-center space-x-1 px-3 py-1 rounded-lg bg-slate-900 border border-slate-800 text-cyan-400 hover:text-cyan-300 font-semibold transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-3">
          ...
          <div className="space-y-2 text-xs">
            {attendanceLogs.length > 0 ? (attendanceLogs.map((log) => (<div key={log._id} className="p-2.5 bg-slate-950 rounded-xl border border-slate-800 flex justify-between items-center">
                  <div>
                ...
                </div>))) : (<p className="text-slate-400 text-xs italic">94.2% Attendance record verified on system database.</p>)}
          </div>
        </div>
      </div>

      {/* Upload Notes Modal */}
      {isNoteModalOpen && (<div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleUploadNote} className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            ...
            <div>
              ...
              <input type="text" required placeholder="e.g. 5th Standard German Chapter 2 Notes" value={noteForm.title} onChange={(e) => setNoteForm({ ...noteForm, title: e.target.value })} className="w-full mt-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-amber-500"/>
            </div>

            <div>
              <label className="text-[11px] font-semibold text-slate-400">Target Grade / Standard / Class</label>
              <input type="text" required placeholder="e.g. 5th Standard (German A1)" value={noteForm.gradeOrClass} onChange={(e) => setNoteForm({ ...noteForm, gradeOrClass: e.target.value })} className="w-full mt-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-amber-500"/>
            </div>

            <div>
              <label className="text-[11px] font-semibold text-slate-400">Description / Instructions</label>
              <textarea rows={2} value={noteForm.description} onChange={(e) => setNoteForm({ ...noteForm, description: e.target.value })} placeholder="Brief summary of the study notes..." className="w-full mt-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-amber-500 resize-none"/>
            </div>

            <div className="flex justify-end gap-2 pt-4 border-t border-slate-800">
              <button type="button" onClick={() => setIsNoteModalOpen(false)} className="px-4 py-2 rounded-xl bg-slate-800 text-xs font-semibold text-slate-300">
                Cancel
              </button>
