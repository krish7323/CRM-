...
import { UserCog, Award, BookOpen, Users, Star, UserPlus, ShieldCheck, Mail, Phone, Lock, X, CheckCircle2 } from 'lucide-react';
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {staffList.map((member) => (
          <div key={member.id} className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-4">
          ...
          <form onSubmit={handleRegisterTeacher} className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full mt-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-cyan-400 font-mono focus:outline-none focus:border-amber-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. Senior Faculty Physics & German"
                value={teacherForm.customRoleTitle}
                onChange={(e) => setTeacherForm({ ...teacherForm, customRoleTitle: e.target.value })}
                className="w-full mt-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-amber-500"
              />
            </div>
