...
import { Calendar, Phone, MessageSquare, Clock, CheckCircle, Search, Filter } from 'lucide-react';

export const FollowupsPage: React.FC = () => {
  const { leads } = useAppStore();
  const [tab, setTab] = useState<'today' | 'overdue' | 'tomorrow' | 'all'>('today');
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex bg-slate-900 border border-slate-800 rounded-xl p-1">
          {(['today', 'overdue', 'tomorrow', 'all'] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold uppercase tracking-wider capitalize transition ${
                tab === t ? 'bg-cyan-500 text-slate-950 shadow' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              {t}
            </button>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {followUpLeads.map((lead) => (
                <tr key={lead._id} className="hover:bg-slate-900/50 transition">
                  <td className="p-4">
                  ...
                  <td className="p-4 text-right space-x-2">
                    <a
                      href={`tel:${lead.phone}`}
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-800 text-slate-200 font-semibold hover:bg-slate-700 transition"
                    >
                      <Phone className="w-3 h-3 text-cyan-400" /> Call
                    </a>
                    <a
                      href={`https://wa.me/${lead.whatsapp.replace(/\D/g, '')}?text=Hallo%20${encodeURIComponent(
                        lead.name
                      )},%20this%20is%20The%20European%20Language%20Hub.%20Following%20up%20on%20your%20${lead.course}%20enquiry!`}
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-600 text-white font-semibold hover:bg-emerald-500 transition"
                    >
