...
import { Calendar, Check, X, AlertCircle, Save, History } from 'lucide-react';
export const AttendancePage = () => {
    ...
    return (<div className="space-y-6 font-sans">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center space-x-2 bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-200">
            ...
            <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="bg-transparent text-slate-100 font-semibold focus:outline-none cursor-pointer"/>
          </div>

          <div className="flex items-center space-x-2 bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-200">
            <span className="text-slate-400 font-medium">Batch:</span>
            <select value={selectedBatch} onChange={(e) => setSelectedBatch(e.target.value)} className="bg-transparent font-bold text-amber-400 focus:outline-none cursor-pointer">
              {batches.map((b) => (<option key={b._id} value={b.code} className="bg-slate-900 text-slate-200">
                  {b.code} ({b.courseName} {b.level})
                </option>))}
            </select>
          </div>

          <button onClick={handleSaveAttendance} className="flex items-center space-x-1.5 bg-gradient-to-r from-emerald-500 to-emerald-600 text-slate-950 font-bold text-xs px-4 py-2 rounded-xl shadow-lg shadow-emerald-500/20 hover:scale-105 transition">
      ...
      {savedSuccess && (<div className="p-3 rounded-xl bg-emerald-950/90 border border-emerald-800 text-emerald-300 text-xs font-bold text-center animate-pulse">
          Attendance log for {selectedBatch} on {selectedDate} permanently saved to database!
        </div>)}

      {/* Existing Log Indicator */}
      {existingLog ? (<div className="p-3 rounded-xl bg-amber-950/40 border border-amber-800/40 flex items-center justify-between text-xs text-amber-300">
        ...
        </div>) : (<div className="p-3 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-between text-xs text-slate-400">
      ...
      <div className="glass-panel rounded-2xl border border-slate-800 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            return (<tr key={std._id} className="hover:bg-slate-900/50 transition">
                    <td className="p-4 font-mono font-bold text-amber-400">{std.studentId}</td>
                    <td className="p-4">
                      <div className="flex items-center space-x-3">
                        <img src={std.photoUrl || 'https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?w=150'} alt={std.name} className="w-8 h-8 rounded-full border border-cyan-500/30 object-cover"/>
                        <div>
                    ...
                    <td className="p-4">
                      <div className="flex items-center justify-center space-x-2">
                        <button type="button" onClick={() => handleStatusChange(std.studentId, 'Present')} className={`px-3 py-1.5 rounded-xl font-bold text-xs flex items-center space-x-1.5 transition ${currentStatus === 'Present'
                    ...
                    : 'bg-slate-900 border border-slate-800 text-slate-400 hover:text-slate-200'}`}>
                        ...
                        <button type="button" onClick={() => handleStatusChange(std.studentId, 'Absent')} className={`px-3 py-1.5 rounded-xl font-bold text-xs flex items-center space-x-1.5 transition ${currentStatus === 'Absent'
                    ...
                    : 'bg-slate-900 border border-slate-800 text-slate-400 hover:text-slate-200'}`}>
                        ...
                        <button type="button" onClick={() => handleStatusChange(std.studentId, 'Leave')} className={`px-3 py-1.5 rounded-xl font-bold text-xs flex items-center space-x-1.5 transition ${currentStatus === 'Leave'
