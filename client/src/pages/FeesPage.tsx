...
import { CircleDollarSign, Receipt, CreditCard, Printer, CheckCircle, Clock, X, AlertCircle } from 'lucide-react';

export const FeesPage: React.FC = () => {
  const { fees, payInstallment } = useAppStore();
  const [selectedFee, setSelectedFee] = useState<FeeRecord | null>(null);
  const [paymentModalFee, setPaymentModalFee] = useState<{ fee: FeeRecord; instNo: number; amount: number } | null>(null);
  const [payMode, setPayMode] = useState<string>('UPI');
  const [refText, setRefText] = useState<string>('GPay-UPI-994182');
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {fees.map((fee) => (
                <tr key={fee._id} className="hover:bg-slate-900/50 transition">
                  ...
                  <td className="p-4 text-right space-x-2">
                    <button
                      ...
                      className="px-3 py-1 rounded-lg bg-slate-800 text-slate-200 hover:bg-slate-700 font-semibold"
                    >
                      Installments
                    </button>
                  </td>
                ...
                <div key={inst.installmentNo} className="bg-slate-950 p-3 rounded-xl border border-slate-800 flex items-center justify-between text-xs">
                  ...
                  {inst.status === 'Paid' ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => alert(`Downloading official PDF receipt for ${selectedFee.studentName} - Installment #${inst.installmentNo}`)}
