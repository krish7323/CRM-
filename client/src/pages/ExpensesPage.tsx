...
import { Receipt, Plus, TrendingDown, CircleDollarSign, X } from 'lucide-react';
      ...
      <div className="glass-panel rounded-2xl border border-slate-800 overflow-hidden">
        ...
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-slate-800/60">
            {expenses.map((exp) => (
              <tr key={exp._id} className="hover:bg-slate-900/50">
          ...
          <form onSubmit={handleSubmit} className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Monthly Lab Maintenance"
                className="w-full mt-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-amber-500"
              />
            </div>
