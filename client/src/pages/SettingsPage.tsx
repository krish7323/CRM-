...
interface ParsedBulkRow {
  ...
  password: string;
  status: 'Valid' | 'Role Hierarchy Exceeded' | 'Duplicate Email';
}

export const SettingsPage: React.FC = () => {
  const { users, customRoles, addUserAccount, updateUserAccount, deleteUserAccount, addCustomRole, deleteCustomRole, currentUser } = useAppStore();
  ...
  // Sample CSV Downloader for 1000+ User Bulk Import
  const downloadSampleCSV = () => {
    const csvContent =
      'Full Name,Email,Phone,Role,SubRoleTitle,Password\n' +
      ...
      'Rohan Patel,rohan.patel@iia.edu,+91 98765 00003,Teacher,Faculty Assistant,password123\n' +
  ...
  // Simulate Bulk File Upload Parsing
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Simulated parsing of 1000+ member batch for 4 roles
    const demoBatch: ParsedBulkRow[] = [
      ...
      { name: 'Siddharth Roy', email: 'siddharth.r@iia.edu', phone: '+91 98765 10003', role: 'Teacher', customRoleTitle: 'Junior Faculty', password: 'password123', status: 'Valid' },
      ...
      { name: 'Rahul Gupta (Duplicate Test)', email: 'admin@elh.edu', phone: '+91 98765 10005', role: 'Student', customRoleTitle: 'Student', password: 'password123', status: 'Duplicate Email' },
    ];
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-100 flex items-center gap-2">
            Hierarchical Roles & Bulk 1000+ User Setup (4-Role Model)
          ...
          <p className="text-xs text-slate-400">
            Simplified Roles: Admin (Rank 1) &gt; Counsellor (Rank 2) &gt; Teacher (Rank 3) &gt; Student (Rank 4)
          </p>
        </div>
        ...
        {creatableRoles.length > 0 && (
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="flex items-center space-x-1.5 bg-gradient-to-r from-teal-500 to-cyan-500 text-slate-950 font-bold text-xs px-3.5 py-2 rounded-xl shadow-lg shadow-teal-500/20 hover:scale-105 transition"
            >
              <FileSpreadsheet className="w-4 h-4" />
              <span>Bulk CSV Import (1000+ Members)</span>
            </button>

            <button
              onClick={() => setIsRoleModalOpen(true)}
              className="flex items-center space-x-1.5 bg-slate-900 border border-slate-800 text-amber-400 font-bold text-xs px-3.5 py-2 rounded-xl hover:bg-slate-800 transition"
            >
      ...
      <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-3">
        <div className="flex items-center justify-between pb-3 border-b border-slate-800">
          ...
          <span className="text-[11px] text-slate-400">Created according to hierarchy limits</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {customRoles.map((cr) => (
            <div key={cr.id} className="bg-slate-950 p-3.5 rounded-xl border border-slate-800 space-y-1 relative group">
              ...
              <div className="pt-2 flex justify-between items-center text-[9px] text-slate-500 border-t border-slate-900">
                ...
                {canManageUser(currentUser.role, cr.parentRole) && (
                  <button
                    onClick={() => deleteCustomRole(cr.id)}
                    className="text-rose-400 hover:text-rose-300 font-bold opacity-0 group-hover:opacity-100 transition"
                  >
                    Delete Sub-Role
                  </button>
                )}
                    ...
                    <tr key={user.id} className="hover:bg-slate-900/50 transition">
                      ...
                      <td className="p-4 space-y-0.5">
                        ...
                        {user.customRoleTitle && (
                          <span className="text-[10px] text-cyan-300 font-semibold italic block">
                            ↳ {user.customRoleTitle}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="p-4">
                        {user.isActive ? (
                          <span className="px-2.5 py-0.5 rounded text-[10px] font-bold bg-emerald-950 text-emerald-400 border border-emerald-800/40 flex items-center gap-1 w-fit">
                            <CheckCircle2 className="w-3 h-3" /> Active
                          </span>
                        ) : (
                          <span className="px-2.5 py-0.5 rounded text-[10px] font-bold bg-rose-950 text-rose-400 border border-rose-800/40 flex items-center gap-1 w-fit">
                        ...
                        {isLowerRank ? (
                          <>
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-slate-800 text-amber-400 hover:bg-slate-700 font-semibold text-[11px] inline-flex items-center gap-1"
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-rose-950 text-rose-400 hover:bg-rose-900 font-semibold text-[11px] inline-flex items-center gap-1"
                          ...
                          <span className="text-[10px] text-slate-500 italic">Protected (Higher/Equal Rank)</span>
                        )}
                      </td>
                    </tr>
                  );
          ...
          <div className="w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div>
                <h3 className="text-sm font-bold text-slate-100 flex items-center gap-2">
                  <FileSpreadsheet className="w-4 h-4 text-teal-400" /> Bulk CSV Importer (1000+ Members & Students)
                </h3>
                <p className="text-xs text-slate-400">Upload CSV to batch-import staff, faculty, counsellors, and students with roles</p>
              </div>
            ...
            <div className="p-4 bg-slate-950 rounded-xl border border-slate-800 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-slate-400">Pre-formatted columns: Full Name, Email, Phone, Role, SubRoleTitle, Password</p>
              </div>
              <button
                onClick={downloadSampleCSV}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-teal-400 text-xs font-semibold transition"
              >
            ...
            <div className="border-2 border-dashed border-slate-800 hover:border-teal-500/50 rounded-2xl p-6 text-center space-y-2 bg-slate-950/50 transition">
              ...
              <p className="text-[10px] text-slate-400">Supports batch import of up to 5,000 records per upload</p>
              <input type="file" accept=".csv" onChange={handleFileUpload} className="hidden" id="csv-file-input" />
              <label
                htmlFor="csv-file-input"
                className="inline-block mt-2 px-4 py-2 rounded-xl bg-teal-500 text-slate-950 font-bold text-xs cursor-pointer hover:bg-teal-400 transition"
              >
                Choose File
              </label>
            </div>
            ...
            {parsedRows.length > 0 && (
              <div className="space-y-3">
                ...
                {importCompleted ? (
                  <div className="p-3 rounded-xl bg-emerald-950/80 border border-emerald-800/60 text-emerald-300 text-xs text-center font-bold">
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-teal-500 text-slate-950 font-bold text-xs hover:bg-teal-400 transition flex items-center gap-2"
                    >
                      {isImporting ? 'Processing Batch...' : 'Confirm Bulk Import (1000+ Members)'}
                    </button>
                  </div>
                )}
              </div>
          ...
          <form onSubmit={handleSaveAccount} className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                value={accountForm.customRoleTitle}
                onChange={(e) => setAccountForm({ ...accountForm, customRoleTitle: e.target.value })}
                className="w-full mt-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-amber-500 font-sans"
              />
            </div>
          ...
          <form onSubmit={handleSaveCustomRole} className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            ...
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. Lead Admissions Officer"
                value={roleForm.name}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setRoleForm({ ...roleForm, parentRole: e.target.value as UserRole })}
                className="w-full mt-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-amber-500"
              >
                {creatableRoles.map((r) => (
                  <option key={r} value={r}>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Responsibilities of this custom sub-role..."
